...
public class CheckChecker {
    ...
    public boolean isCheck(Piece king, Coord destination){
        if ((BoardManager.getInstance().getBoardInstance().getPieceOn(destination) != null)&&(BoardManager.getInstance().getBoardInstance().getPieceOn(destination).getType()== ChessPieceEnum.king)){
            //null exception, king check
